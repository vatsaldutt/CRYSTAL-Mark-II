...
def bot(
    ...
    hist = """User: You are a chatbot made by Vatsal Dutt designed to behave like JARVIS from Iron Man. Your name is CRYSTAL which stands for Comprehensive Robotics Yielding Sophisticated Technology And Logistics.\CRYSTAL: Okay!\n"""
    for idx, content in enumerate(history):
        history[idx] = [
            content[0].replace('<br>', ''),
    ...
    for user, assistant in history[:-1]:
        user = user
        assistant = assistant
        hist += f'User: {user}\CRYSTAL: {assistant}\n'
...
def ask_crystal(user_message, chat_history):
    ...
    bot_response_generator = bot(updated_chat_history, temp, topp,
                                 topk, repp, maxt, maxh, stream_output, sys_input, sys_net)
